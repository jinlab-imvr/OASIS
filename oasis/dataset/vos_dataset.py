...
class VOSMergeTrainDataset(Dataset):
    ...
    def __init__(
        self, data_configs, seq_length=3, max_num_obj=3, size=480, merge_probability=0.0
    ):
        ...
        self.max_seed_trials = (
            5  # number of attempts at changing the initial seed frame
        )
        self.max_seq_trials = (
            100  # number of attempts at generating a sequence from the seed frame
        )
    ...
    def _get_sample(self, idx=None):
        ...
        while True:
            ...
            empty_masks = (
                config["empty_masks"][video] if config["empty_masks"] else None
            )
            ...
            for seed_trial in range(self.max_seed_trials):
                ...
                for seq_trial in range(self.max_seq_trials):
                    ...
                    if (
                        empty_masks is None
                        or frames[sampled_frames[0]][:-4] not in empty_masks
                    ):
                ...
                for i, f_idx in enumerate(frames_idx):
                    ...
                    if i == 0:
                        for crop_trial in range(self.max_crop_trials):
                            ...
                            this_gt = Image.open(path.join(gt_path, png_name)).convert(
                                "P"
                            )
                            this_gt_gray = this_gt.convert("L")
                            this_gt = self.sequence_mask_dual_transform(this_gt)
                            this_gt = np.array(this_gt)

                            reseed(sequence_seed)
                            this_gt_binary = ImageOps.autocontrast(
                                this_gt_gray
                            )  # This adjusts the contrast automatically
                            # Apply edge detection using the built-in EDGE filter
                            this_gt_edges = this_gt_binary.filter(
                                ImageFilter.FIND_EDGES
                            )
                            this_gt_edges = self.sequence_mask_dual_transform(
                                this_gt_edges
                            )
                            this_gt_edges = np.array(this_gt_edges)
                    ...
                    this_im_gray = this_im.convert("L")
                    ...
                    # this_im_edges = self.sequence_image_only_transform(this_im_edges)
                    # this_im_edges = self.frame_image_transform(this_im_edges)
                ...
                return info, images, masks, edges, im_edges
    ...
    def __getitem__(self, idx):
        ...
        info, images, masks, edges, im_edges = self._get_sample(idx)
        ...
        if num_labels < self.max_num_obj and np.random.rand() < self.merge_probability:
            _, images2, masks2, edges2, im_edges2 = self._get_sample()
            ...
            for l2 in labels2:
                ...
                blur_masks = cv2.GaussianBlur(blur_masks, [5, 5], 1.0).transpose(
                    2, 0, 1
                )[:, None]
                images = images * (1 - blur_masks) + images2 * blur_masks

                edges2 = np.clip(edges2, 0.0, 1.0, dtype=float)
                edges = np.logical_or(edges, edges2).astype(float)

                im_edges2 = torch.clamp(im_edges2, 0.0, 1.0)
                im_edges = torch.logical_or(im_edges, im_edges2).to(dtype=torch.float)
