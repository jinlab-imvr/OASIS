...
from oasis.model.oasis import OASIS

log = logging.getLogger()


class OasisTrainWrapper(OASIS):
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig):
        ...
        self.move_t_out_of_batch = Rearrange(
            "(b t) c h w -> b t c h w", t=self.seq_length
        )
        self.move_t_from_batch_to_volume = Rearrange(
            "(b t) c h w -> b c t h w", t=self.seq_length
        )
        self.move_t_to_batch = Rearrange("b t c h w -> (b t) c h w", t=self.seq_length)

    def forward(self, data: Dict, current_epoch: int):
        out = {}
        frames = data["rgb"]
        first_frame_gt = data["first_frame_gt"].float()
        edges = data["edges"].unsqueeze(2)
        im_edges = data["im_edges"]
        ...
        with torch.amp.autocast("cuda", enabled=self.use_amp):
            ...
            # im_edges = im_edges.view(b * seq_length, *im_edges.shape[2:])
            ms_feat, pix_feat = self.encode_image(frames_flat)
            # ms_feat = self.feat_fusion(ms_feat, im_edges)
            # edges = self.edge_net(frames_flat)

            # for ti in range(0, seq_length):
            # with torch.amp.autocast("cuda", enabled=False):
            #     edges = self.decoder_edge(
            #         self.feat_fusion([f.float() for f in ms_feat], im_edges)
            #     )

            # out[f"edges_{ti}"] = edges

            # ms_feat = self.feat_edge_fusion(ms_feat, edges)
            ...
            # edges = self.move_t_out_of_batch(edges)

            # for ti in range(0, seq_length):
            #     out[f"edges_{ti}"] = edges[:, ti]

            # zero-init sensory
            sensory = torch.zeros(
                (b, num_objects, self.sensory_dim, h, w), device=frames.device
            )
            ...
            for ti in range(1, seq_length):
                with torch.amp.autocast("cuda", enabled=False):
                    edges = self.decoder_edge(
                        self.feat_fusion(
                            [f.float() for f in get_ms_feat_ti(ti)], im_edges[:, ti]
                        ),
                        torch.sum(sensory, dim=1),
                    )
                out[f"edges_{ti}"] = edges
                if ti <= self.num_ref_frames:
                    ...
                    ref_keys = keys[:, :, :ti]
                    ref_shrinkages = (
                        shrinkages[:, :, :ti] if shrinkages is not None else None
                    )
                else:
                    ...
                    ref_keys = torch.stack(
                        [keys[bi, :, ridx[bi]] for bi in range(b)], 0
                    )
                ...
                sensory, logits, masks = self.segment(
                    self.feat_edge_fusion(get_ms_feat_ti(ti), edges),
                    readout,
                    sensory,
                    selector=selector,
                )
