...
from oasis.model.train_wrapper import OasisTrainWrapper
from oasis.model.utils.parameter_groups import get_parameter_groups
from oasis.model.losses import LossComputer
from oasis.utils.log_integrator import Integrator
from oasis.utils.image_saver import vis
from oasis.utils.logger import TensorboardLogger
from oasis.utils.time_estimator import TimeEstimator


class Trainer:
    def __init__(
        self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path
    ):
        ...
        self.oasis = nn.parallel.DistributedDataParallel(
            OasisTrainWrapper(cfg, stage_cfg).cuda(),
        ...
        self.log.log_string(
            "model_size",
            str(sum([param.nelement() for param in self.oasis.parameters()])),
        )
        self.log.log_string(
            ...
            str(
                sum(
                    [
                        param.nelement()
                        for param in filter(
                            lambda p: p.requires_grad, self.oasis.parameters()
                        )
                    ]
                )
            ),
        ...
        parameter_groups = get_parameter_groups(
            self.oasis, stage_cfg, print_log=(local_rank == 0)
        )
    ...
    def do_pass(self, data, it=0, current_epoch=0):
        ...
        out = self.oasis(data, current_epoch)

        num_filled_objects = out["num_filled_objects"]
        if self._is_train:
            losses = self.loss_computer.compute(
                {**data, **out}, num_filled_objects, current_epoch
            )
            ...
            if self._is_train:
                if (
                    self.local_rank == 0
                    and it % self.log_image_interval == 0
                    and it != 0
                ):
                    images = {**data, **out}
                    self.log.log_image(
                        self.stage,
                        "vis",
                        vis(images, self.size, num_filled_objects),
                        it,
                    )
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.oasis.parameters(), self.clip_grad_norm
            )
    ...
    def save_weights(self, it, save_copy=False):
        ...
        if save_copy:
            model_path = os.path.join(
                self.run_path, f"{self.exp_id}_{self.stage}_{it}.pth"
            )
            torch.save(self.oasis.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it, save_copy=False):
        ...
        checkpoint = {
            "it": it,
            "weights": self.oasis.module.state_dict(),
        ...
        if save_copy:
            model_path = os.path.join(
                self.run_path, f"{self.exp_id}_{self.stage}_ckpt_{it}.pth"
            )
            torch.save(checkpoint, model_path)
            self.log.info(f"Checkpoint saved to {model_path}.")

        model_path = os.path.join(
            self.run_path, f"{self.exp_id}_{self.stage}_ckpt_last.pth"
        )
        torch.save(checkpoint, model_path)
        self.log.info(f"Checkpoint saved to {model_path}.")
    ...
    def load_weights_in_memory(self, src_dict):
        self.oasis.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.oasis.module.state_dict()
