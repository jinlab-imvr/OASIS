...
from oasis.model.group_modules import *
from oasis.model.utils import resnet
from oasis.model.modules import *
...
class EdgeDecoder(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        if self.ms_dims[0] == self.sensory_dim:
            self.up_sense = nn.Identity()
        else:
            self.up_sense = ConvLayer(self.sensory_dim, self.ms_dims[0])

        self.conv_scale1 = ConvLayer(
            self.ms_dims[0], self.ms_dims[0]
        )  # 30x30
        self.conv_scale2 = ConvLayer(
            self.ms_dims[1], self.ms_dims[1]
        )  # 60x60
        self.conv_scale3 = ConvLayer(
            self.ms_dims[2], self.ms_dims[2]
        )  # 120x120
        self.up1 = UpSample(self.ms_dims[0])
        self.decoder1 = ConvLayer(self.ms_dims[0], self.ms_dims[1])

        self.up2 = UpSample(self.ms_dims[1])
        self.decoder2 = ConvLayer(self.ms_dims[1], self.ms_dims[2])

        self.up3 = UpSample(self.ms_dims[2])
        self.up4 = UpSample(self.ms_dims[2] // 2)
        self.final_conv = nn.Conv2d(self.ms_dims[2] // 4, 1, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, ms_image_feat, sensory):
        f1 = ms_image_feat[0]  # 256 c
        f2 = ms_image_feat[1]  # 128 c
        rc_f2 = f2
        f3 = ms_image_feat[2]  # 64 c
        rc_f3 = f3

        # Process each scale + self.up_sense(sensory)
        f1 = self.conv_scale1(
            f1 + self.up_sense(sensory)
        )
        f2 = self.conv_scale2(f2)
        f3 = self.conv_scale3(f3)

        # Decoder pathway with upsampling
        up1 = self.up1(f1)  # Upsample
        merge1 = torch.cat((up1, f2), dim=1)
        dec1 = self.decoder1(merge1)

        dec1 = rc_f2 + dec1
        dec1 = F.leaky_relu(dec1)

        up2 = self.up2(dec1)  # Upsample
        merge2 = torch.cat((up2, f3), dim=1)
        dec2 = self.decoder2(merge2)

        dec2 = rc_f3 + dec2
        dec2 = F.leaky_relu(dec2)
