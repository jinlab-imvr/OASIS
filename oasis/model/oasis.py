...
from oasis.model.big_modules import *
from oasis.model.aux_modules import AuxComputer
from oasis.model.utils.memory_utils import *
from oasis.model.transformer.object_transformer import QueryTransformer
from oasis.model.transformer.object_summarizer import ObjectSummarizer
from oasis.utils.tensor_utils import aggregate
...
class OASIS(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        self.edge_factor = 0.5
        self.edge_feat_factor = 1.0
        ...
        self.edge_decoder = EdgeDecoder(model_cfg)
        self.pixel_fuser = PixelFeatureFuser(model_cfg, single_object=single_object)

        # self.distri_net = DisNet(model_cfg.dis_inchannels)
    ...
    def feat_fusion(
        self, img_embeddings: List[torch.Tensor], edges_tensor: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        out = []
        for img_embedding in img_embeddings:
            edges_tensor = F.interpolate(
                edges_tensor,
                size=img_embedding.shape[-2:],
                mode="nearest-exact",
            )
            img_embedding = torch.mul(
                img_embedding, (self.edge_factor * edges_tensor) + 1.0
            ).to(img_embedding.dtype)
            out.append(img_embedding)
        return out

    def feat_edge_fusion(
        self, img_embeddings: List[torch.Tensor], edges_tensor: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        out = []
        for img_embedding in img_embeddings:
            ...
            img_embedding = torch.mul(
                img_embedding, (self.edge_feat_factor * edges_tensor) + 1.0
            ).to(img_embedding.dtype)
            out.append(img_embedding)
        return out

    def decoder_edge(
        self, img_embeddings: List[torch.Tensor], sensory: torch.Tensor
    ) -> torch.Tensor:
        out = self.edge_decoder(img_embeddings, sensory)
        return out
