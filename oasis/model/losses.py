...
from oasis.utils.point_features import (
    calculate_uncertainty,
    point_sample,
    get_uncertain_point_coords_with_randomness,
)
from oasis.utils.tensor_utils import cls_to_one_hot
from oasis.model.losses_edl import edl_digamma_loss
...
def digma_edl_loss(
    logits: torch.Tensor, soft_gt: torch.Tensor, current_epoch: int
) -> torch.Tensor:
    # logits: T*C*num_points
    # soft_gt: T*C*num_points
    device = logits.device
    logits = rearrange(logits, "t c n -> (t n) c")
    soft_gt = rearrange(soft_gt, "t c n -> (t n) c")
    num_classes = soft_gt.shape[1]
    loss = 0.01 * edl_digamma_loss(
        logits, soft_gt, current_epoch, num_classes, 5, device
    )
    return loss
...
class LossComputer:
    ...
    def mask_loss(
        self, logits: torch.Tensor, soft_gt: torch.Tensor, current_epoch: int
        ...
        loss_edl = digma_edl_loss(point_logits, point_labels, current_epoch)

        return loss_ce, loss_dice, loss_edl

    def compute(
        self, data: Dict[str, torch.Tensor], num_objects: List[int], current_epoch: int
    ) -> Dict[str, torch.Tensor]:
        batch_size, num_frames = data["rgb"].shape[:2]
        losses = defaultdict(float)
        t_range = range(1, num_frames)
        edges_range = range(0, num_frames)

        for bi in range(batch_size):
            logits = torch.stack(
                [data[f"logits_{ti}"][bi, : num_objects[bi] + 1] for ti in t_range],
                dim=0,
            )
            edges_logits = torch.stack(
                [data[f"edges_{fi}"][bi] for fi in t_range],
                dim=0,
            ).squeeze(1)
            edges_gt = data["edges"][bi, 1:]  # remove gt for the first frame
            cls_gt = data["cls_gt"][bi, 1:]  # remove gt for the first frame
            soft_gt = cls_to_one_hot(cls_gt, num_objects[bi])

            loss_ce, loss_dice, loss_edl = self.mask_loss(
                logits, soft_gt, current_epoch
            )
            losses["loss_ce"] += loss_ce / batch_size
            losses["loss_dice"] += loss_dice / batch_size
            losses["loss_edge"] += (
                self.edge_loss(edges_logits, edges_gt.float()) / batch_size
            )
            losses["loss_edl"] += loss_edl / batch_size
            ...
            if "sensory_logits" in aux[0]:
                ...
                loss_ce, loss_dice, _ = self.mask_loss(
                    sensory_log, soft_gt, current_epoch
                )
                losses["aux_sensory_ce"] += loss_ce / batch_size * self.sensory_weight
                losses["aux_sensory_dice"] += (
                    loss_dice / batch_size * self.sensory_weight
                )
            if "q_logits" in aux[0]:
                ...
                for l in range(num_levels):
                    query_log = torch.stack(
                        [a["q_logits"][bi, : num_objects[bi] + 1, l] for a in aux],
                        dim=0,
                    )
                    loss_ce, loss_dice, _ = self.mask_loss(
                        query_log, soft_gt, current_epoch
                    )
                    losses[f"aux_query_ce_l{l}"] += (
                        loss_ce / batch_size * self.query_weight
                    )
                    losses[f"aux_query_dice_l{l}"] += (
                        loss_dice / batch_size * self.query_weight
                    )
