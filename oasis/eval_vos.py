...
from oasis.inference.data.vos_test_dataset import VOSTestDataset
from oasis.inference.data.burst_test_dataset import BURSTTestDataset
from oasis.model.oasis import OASIS
from oasis.inference.inference_core import InferenceCore
from oasis.inference.utils.results_utils import ResultSaver, make_zip
from oasis.inference.utils.burst_utils import BURSTResultHandler
from oasis.inference.utils.args_utils import get_dataset_cfg
...
def eval_vos(cfg: DictConfig):
    ...
    oasis = OASIS(cfg).cuda().eval()
    if cfg.weights is not None:
        model_weights = torch.load(cfg.weights)
        oasis.load_weights(model_weights)
    else:
    ...
    for vid_reader in pbar:
        ...
        try:
            processor = InferenceCore(oasis, cfg=cfg)
            saver = ResultSaver(
                mask_output_root,
                vid_name,
