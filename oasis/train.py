...
from oasis.model.trainer import Trainer
from oasis.dataset.setup_training_data import (
...
def train(cfg: DictConfig):
    ...
    for stage in stages:
        ...
        if "max_skip_schedule" in stage_cfg:
            ...
            change_skip_iter = [
                round(total_iterations * f) for f in increase_skip_fraction
            ]
            ...
            change_skip_iter = []
        change_skip_iter.append(
            total_iterations + 1
        )  # dummy value to avoid out of index error

        # setup datasets
        if stage == "pre_training":
            dataset, sampler, loader = setup_pre_training_datasets(cfg)
        elif stage == "main_training":
            dataset, sampler, loader = setup_main_training_datasets(
                cfg, max_skip_schedule[0]
            )
        ...
        try:
            ...
            while curr_iter < total_iterations:
                ...
                for data in loader:
                    ...
                    trainer.do_pass(data, curr_iter, current_epoch)
                    curr_iter += 1
