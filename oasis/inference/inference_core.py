...
from oasis.inference.memory_manager import MemoryManager
from oasis.inference.object_manager import ObjectManager
from oasis.inference.image_feature_store import ImageFeatureStore
from oasis.model.oasis import OASIS
from oasis.utils.tensor_utils import pad_divide_by, unpad, aggregate
        ...
        *,
        ...
        if self.max_internal_size > 0:
            ...
            if min_side > self.max_internal_size:
                ...
                im_edges = F.interpolate(
                    im_edges.unsqueeze(0),
                    size=(new_h, new_w),
                    mode="nearest",
                    align_corners=False,
                )[0]
        ...
        im_edges, _ = pad_divide_by(im_edges, 16)

        image = image.unsqueeze(0)  # add the batch dimension
        im_edges = im_edges.unsqueeze(0)
        if self.flip_aug:
            image = torch.cat([image, torch.flip(image, dims=[-1])], dim=0)
            im_edges = torch.flip(im_edges, dims=[-1])
        ...
        if need_segment:
            edges = self.network.decoder_edge(
                self.network.feat_fusion(ms_feat, im_edges),
                torch.sum(
                    self.memory.get_sensory(self.object_manager.all_obj_ids), dim=1
                ),
            )
            # edges = self.network.decoder_edge(image, None)[-1]
            # print(edges.shape, im_edges.shape)
            # save_image([edges.squeeze(0), im_edges.squeeze(0)], "vis/edges.png")
            # save_image([edges.squeeze(0)], "vis/edges.pdf")
            # save_image([image.squeeze(0)], "vis/image.png")
            # save_image(
            #     [
            #         F.interpolate(
            #             edges,
            #             size=(ms_feat[2].shape[-2], ms_feat[2].shape[-1]),
            #             mode="nearest-exact",
            #         ).squeeze(0)
            #     ],
            #     "vis/edge_resize.png",
            ...
            ms_feat = self.network.feat_edge_fusion(ms_feat, edges)
