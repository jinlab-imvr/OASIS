...
class VideoReader(Dataset):
        ...
        *,
        ...
        if first_mask.mode == "P":
            ...
            raise NotImplementedError(
                f"Unknown mode {first_mask.mode} in {self.first_mask_frame}."
            )
    ...
    def __getitem__(self, idx):
        ...
        im_gray_np = np.array(img_gray)
        im_edges = cv2.Canny(im_gray_np, 50, 200, L2gradient=True)
        ...
        im_edges = self.im_transform(im_edges)
        if resize_needed:
            img = self.im_resize(img)
            im_edges = self.im_resize(im_edges)
        ...
        if load_mask:
            ...
            if path.exists(mask_path):
                ...
                if self.use_long_id:
                    ...
                    mask = (
                        mask[:, :, 0] + mask[:, :, 1] * 256 + mask[:, :, 2] * 256 * 256
                    )
                else:
                    assert (
                        len(mask.shape) == 2
                    ), "Single channel masks should have 2 dimensions"
                    pass
